...
const Order = {
    create: async (orderRequest) => {
        ...
        try {
            ...
            const { service_type, payment_method, total_amount, platform } = orderData;

            // 1. Create Order
            const orderQuery = `
              INSERT INTO orders (user_id, platform, status, service_type, payment_method, total_amount)
              VALUES ($1, $2, $3, $4, $5, $6)
              RETURNING *;
          `;
            const orderValues = [user_id, platform || 'web', 'pending', service_type, payment_method, total_amount];
    ...
    getStats: async () => {
        ...
        const pendingQuery = `SELECT COUNT(*) as count FROM orders WHERE status = 'created' OR status = 'pending'`;
    ...
    findPending: async () => {
        const query = `
            ...
            LEFT JOIN order_items oi ON o.id = oi.order_id
            LEFT JOIN foods f ON oi.food_id = f.id
            WHERE o.status IN ('pending', 'accepted', 'preparing', 'ready', 'created', 'confirmed')
            GROUP BY o.id
