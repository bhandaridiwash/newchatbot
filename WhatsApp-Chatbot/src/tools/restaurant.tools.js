...
async function createOrder(userWaId) {
  const res = await db.query(
    'INSERT INTO orders (customer_id, status, created_at) VALUES ($1, $2, NOW()) RETURNING id, status, created_at',
    [userWaId, 'created']
  );
  return res.rows[0];
}
...
async function updateDeliveryAddress(orderId, address) {
  const res = await db.query(
    'UPDATE orders SET delivery_address = $1 WHERE id = $2 RETURNING id, delivery_address',
    [address, orderId]
  );
  return res.rows[0];
}
 ...
 */
/**
 * Create a new reservation for dine-in
 * @param {string} customerId - WhatsApp user ID
 * @param {number} partySize - Number of people
 * @param {Date|string} dineTime - Reservation timestamp
 * @returns {Promise<Object>} - Created reservation
 */
async function createReservation(customerId, partySize, dineTime) {
  try {
    // Parse dineTime to get date and time
    const dateObj = new Date(dineTime);
    const reservationDate = dateObj.toISOString().split('T')[0]; // YYYY-MM-DD
    const reservationTime = dateObj.toTimeString().split(' ')[0].substring(0, 5); // HH:MM

    const res = await db.query(
      `INSERT INTO reservations (customer_id, platform, party_size, reservation_date, reservation_time, status, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, NOW())
       RETURNING id, customer_id, party_size, reservation_date, reservation_time, status, created_at`,
      [customerId, 'whatsapp', partySize, reservationDate, reservationTime, 'confirmed']
    );
    return res.rows[0];
  } catch (error) {
...
async function updateReservation(orderId, partySize, arrivalTime) {
  const res = await db.query(
    // NOTE: This now refers to the reservations table conceptually, but if updating an existing order with dine-in details:
    // The new schema doesn't have party_size in orders. 
    // For now we will assume this function might be deprecated or needs to act on a linked reservation.
    // However, to keep it simple and not break flow, we explicitly won't update columns that don't exist.
    // FIXME: Schema change removed party_size/arrival_time from orders.
    'SELECT id, status FROM orders WHERE id = $3', // placeholder to avoid error
...
async function updateDepositStatus(orderId, amount, status) {
  const res = await db.query(
    // FIXME: Schema change removed deposit columns from orders.
    'SELECT id, status FROM orders WHERE id = $3', // placeholder
...
async function finalizeOrderFromCart(userId, cart, orderDetails = {}) {
  ...
  try {
    ...
    const orderRes = await db.query(
      `INSERT INTO orders (customer_id, status, service_type, delivery_address, payment_method, platform, created_at) 
       VALUES ($1, $2, $3, $4, $5, $6, NOW()) 
       RETURNING id, status, created_at`,
      [
        userId,
        'created',
        orderDetails.service_type || 'pickup',
        orderDetails.delivery_address || null,
        dbPaymentMethod,
